...
import { CRYPTO_COINS_LIST } from '../../../dummy/CryptoCoins';
...
const CustomForm: React.FC = () => {
  ...
  const debouncedSearch = useDebounce(onCoinSearch, 1000);
    ...
    if (optionType === 'coins') {
      return optionListItems.map((coin: any) => (
        <Option key={coin?.id} value={coin?.id} label={coin?.name}>
          <Row gutter={[8, 24]} align="middle" style={{ padding: '4px 0px' }}>
            <Col>
              <Image
            ...
            <Col>
              <span style={{ color: '#808A9D' }}>{coin?.symbol?.toUpperCase()}</span>
            </Col>
          </Row>
        </Option>
      ));
        ...
        >
          ...
          >
            <Select
              ...
              suffixIcon={loading ? <CustomLoader /> : <DownOutlined />}
            >
